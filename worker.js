...
// 支持多账号签到，使用 JSON 格式的环境变量，支持 Telegram / 钉钉通知

// 常量定义
const API_BASE_URL = "https://glados.cloud/api";
const API_ENDPOINTS = {
...
let dingtalkWebhook = ''; // DingTalk Webhook
let dingtalkSecret = ''; // DingTalk Secret
let checkinResults = [];
let accountStatus = [];
let pointsHistory = [];
let workerUrl = '';
    ...
    async fetch(request, env, ctx) {
        ...
        switch (url.pathname) {
            ...
            case "/ding":
                await performAllCheckins();
                await checkAllAccountStatus();
                await sendDingTalkMessage(request);
                return createResponse("已执行签到并发送结果到钉钉");
...
async function initializeVariables(env) {
    ...
    // 设置钉钉信息
    dingtalkWebhook = (env.DINGTALK_WEBHOOK || dingtalkWebhook || '').trim();
    dingtalkSecret = (env.DINGTALK_SECRET || dingtalkSecret || '').trim();
...
function buildDingtalkText(request) {
    const formattedTime = formatBeijingTime();
    const baseUrl = request ? getWorkerUrl(request) : workerUrl;
    const chartUrl = baseUrl ? `${baseUrl}/checkinChart` : '';
    const lines = [
        '### GLaDOS 签到报告',
        `> ${formattedTime}`,
        ''
    ...
    if (checkinResults.length > 0) {
        lines.push('**签到结果**');
        checkinResults.forEach((item) => lines.push(`- ${stripHtml(item)}`));
        lines.push('');
    }

    if (accountStatus.length > 0) {
        lines.push('**账号状态**');
        accountStatus.forEach((item) => lines.push(`- ${stripHtml(item)}`));
        lines.push('');
    }

    lines.push(`> ✅ 共完成 ${accounts.length} 个账号的签到任务`);
    if (chartUrl) {
        lines.push(`> 📊 积分历史图表：${chartUrl}`);
    }

    return lines.join('\n');
}

async function sendDingTalkMessage(request) {
    if (!dingtalkWebhook) {
        return;
    }
    ...
    if (dingtalkSecret) {
        ...
        const sign = await dingtalkSign(dingtalkSecret, timestamp);
        const sep = url.includes('?') ? '&' : '?';
        url = `${url}${sep}timestamp=${timestamp}&sign=${sign}`;
    }

    const text = buildDingtalkText(request);
    const payload = {
        msgtype: 'markdown',
        markdown: {
            title: 'GLaDOS 签到报告',
    ...
    try {
        ...
        if (!resp.ok) {
            console.error('钉钉推送失败:', resp.status, resp.statusText);
            return;
        }
        const data = await resp.json();
        if (data.errcode !== 0) {
            console.error('钉钉推送失败:', data.errmsg || 'unknown error');
        }
    } catch (error) {
        console.error('钉钉推送异常:', error);
    }
}
