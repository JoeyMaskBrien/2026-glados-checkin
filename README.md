...
| `DINGTALK_WEBHOOK` | 钉钉机器人 Webhook（见下方） | ❌ 否 |
| `DINGTALK_SECRET` | 钉钉机器人加签密钥（启用加签时） | ❌ 否 |
...
如果你希望签到结果推送到钉钉群，请配置自定义机器人：

1. 在钉钉群聊中添加 **自定义机器人**（Webhook 模式）
2. 复制 Webhook 地址，添加到 GitHub Secrets，Name 填 `DINGTALK_WEBHOOK`
3. 如果启用了 **加签** 安全设置，将密钥填入 `DINGTALK_SECRET`
4. 如果启用了 **关键词** 安全设置，确保关键词出现在推送内容（默认标题含 "GLaDOS"）
...
钉钉机器人推送为 Markdown 文本，标题类似 `GLaDOS签到: 成功x/y`。
...
如果你希望在浏览器里完成签到，可以使用 `userscripts/glados-checkin.user.js`：

1. 安装 Tampermonkey 或 ScriptCat
2. 导入 `userscripts/glados-checkin.user.js`
3. 登录 GLaDOS，并保持页面/浏览器开启
4. 脚本会按本地时间 09:30 / 21:30 自动签到

> ⚠️ 浏览器关闭、Cookie 失效或脚本被禁用时将无法自动签到。

---

## ☁️ Cloudflare Worker 部署（可选）

如果你希望用 CF Worker 托管签到，可直接使用 `worker.js`：

1. 在 Cloudflare Dashboard 创建 Worker，进入在线编辑器
2. 把 `worker.js` 全部内容粘贴进去并部署
3. 在 **Settings → Variables** 配置以下变量（建议用 Secrets）：

| 变量 | 说明 | 必需 |
|------|------|------|
| `GLADOS_ACCOUNTS` | 账号 JSON（见下方示例） | ✅ 是 |
| `DINGTALK_WEBHOOK` | 钉钉机器人 Webhook | ❌ 否 |
| `DINGTALK_SECRET` | 钉钉机器人加签密钥 | ❌ 否 |
| `TGTOKEN` | Telegram Bot Token | ❌ 否 |
| `TGID` | Telegram Chat ID | ❌ 否 |
| `WORKER_URL` | Worker 访问地址（用于图表链接） | ❌ 否 |
...
**定时触发（Cron Triggers）**：
- 北京时间 09:30 / 21:30
- CF Cron 使用 UTC，请设置为 `30 1 * * *` 和 `30 13 * * *`
...
| `userscripts/glados-checkin.user.js` | 油猴/脚本猫定时脚本 |
| `images/` | 教程截图 |
